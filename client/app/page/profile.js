(function () {
    ...
    function profileCtrl($q, $scope, $http, $filter, $uibModal, $mdDialog, config, appConstants, service, materials, userbankdetModel, userlocpreferModel, usercompanyModel, userskillModel, userModel, userexpModel, userproofModel, commonModel) {
        ...
        $scope.isapproved = sessionStorage.userstatusid == "3" ? true : false;
        $scope.isupdate = sessionStorage.userstatusid == "6" ? true : false;
        $scope.isrejected = sessionStorage.userstatusid == "4" ? true : false;
        ...
        $scope.enableEdit = function (type) {
            if (type == 'agency') {
                ...
                $scope.isSkillEditing = true;
            }
            else if (type == 'proof') {
                $scope.isProofEditing = true;
            }           
            else if (type == 'bank') {
                $scope.isBankEditing = true;
            }
            else if (type == 'location') {
                $scope.isLocationEditing = true;
            }
            else if (type == 'experience') {
                $scope.isExperienceEditing = true;
            }
        }
        $scope.cancelEdit = function (type) {
            if (type == 'agency') {
                ...
                $scope.isSkillEditing = false;
            }
            else if (type == 'proof') {
                $scope.isProofEditing = false;
            }            
            else if (type == 'bank') {
                $scope.isBankEditing = false;
            }
            else if (type == 'location') {
                $scope.isLocationEditing = false;
            }
            else if (type == 'experience') {
                $scope.isExperienceEditing = false;
            }           
        }
        ...
        function getuserdetail(userid) {
            ...
            result.then(function (resolve) {
                ...
                $scope.isEditing = true;
                    ...
                    if ($scope.user.isbankaccept) {
                        $scope.isBankEditing = false;
                    }
                    if ($scope.user.isproofaccept) {
                        $scope.isProofEditing = false;
                    }
                    if ($scope.user.isworkaccept) {
                        $scope.isExperienceEditing = false;
                    }
                    if ($scope.user.islocaccept) {
                        $scope.isLocationEditing = false;
                    }
                    if ($scope.user.isskillaccept) {
                        $scope.isSkillEditing = false;
                    }
                    if ($scope.user.isdetailaccept) {
                        $scope.isEditing = false;
                    }
        ...
        $scope.getarealist = function(item, index, area, list){ 
            if(list[index].areaid.length == 0)       
                list[index].areaid = area.split(",");
            list[index].islocall = false;
            list[index].listarea = $filter('filter')($scope.listarea, { cityid: item }, true);
        }
