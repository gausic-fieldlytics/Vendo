(function () {
    ...
    angular.module('app.table').controller('CollectedItemReportCtrl', ['$q', '$scope', '$http', '$filter', '$mdDialog', 'config', 'appConstants', 'service', 'materials', 'reportModel', 'commonModel','targetModel', CollectedItemReportCtrl]);

    function CollectedItemReportCtrl($q, $scope, $http, $filter, $mdDialog, config, appConstants, service, materials, reportModel, commonModel, targetModel) {
        ...
        $scope.loadDropDown = function (id, varId) {
            ...
            var ddlistModel = reportModel.collecteditemddlist;
      ...
      //  $scope.loadDropDown(0, '');
        ...
        $scope.runReport = function () {
           ...
           // obj.skillid = $scope.taskreport.skillid == undefined ? 0 : $scope.taskreport.skillid;
            var result = service.serverPost(config.url_getcollecteditemreport, reportModel.collecteditemrunReport, "", obj);
            result.then(function (resolve) {
        ...
        $scope.cols = ["Source", "Booking Id", "Customer", "Pick Up", "Drop", "Status", "Distance", "Duration", "Amount"];
        $scope.colstitle = ["bookingsourcename", "bookingrefno", "customername", "pickupdate", "dropdate", "bookingstatusname", "distance", "duration", "amount"];
        ...
        $scope.exceller = function () {
            ...
            $scope.fileName = "Booing Report";
            $scope.exportData = [];
            var headtitle ="Booing Report";
                        ...
                        for (var key in selectObjec) {
                            if (key === $scope.colstitle[m]) {
                                switch (key) {
                                    ...
                                    case "pickupdate":
                                    case "dropdate":
                                        var _val = materials.stockconvertdatetime(new Date(selectObjec[key]), "C");
        ...
        $scope.exceldownload = function () {
            ...
            saveAs(new Blob([s2ab(wbout)], { type: "application/octet-stream" }), $scope.fileName + 'test.xlsx');
