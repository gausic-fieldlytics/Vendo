(function () {
    ...
    angular.module('app.table').controller('TaxCtrl', ['$q', '$scope', '$http', '$filter', '$mdDialog', 'config', 'appConstants', 'service', 'materials', 'taxModel', 'commonModel', TaxCtrl]);

    function TaxCtrl($q, $scope, $http, $filter, $mdDialog, config, appConstants, service, materials, taxModel, commonModel) {
		var selectedId = 0;
        var original;
        var init;
        this.tax = taxModel.add;

        $scope.access_permission = {};
        $scope.access_permission = materials.getaccess(15);

        $scope.deleteTax = function (obj) {

            var confirm = materials.deleteConfirm(appConstants.tax);
            $mdDialog.show(confirm).then(function () {
                var delObj = {
                    id: obj.id
                };
                var result = service.serverDelete(config.urlDeleteTax, taxModel.delete, commonModel.trans, delObj);
                result.then(function (resolve) {
                    
                    if (resolve.ResponseStatus == "OK") {
                        materials.displayToast(appConstants.successClass, appConstants.tax + ' ' + appConstants.deleteMsg);
                        getAllTax(selectedId);
                    }
                    else {
        ...
        $scope.editTax = function (rowData) {

            $scope.show = 2;
            $scope.tax = angular.copy(rowData);
            original = angular.copy($scope.tax);
        ...
        $scope.canRevert = function () {
            return !angular.equals($scope.tax, original) || !$scope.material_login_form.$pristine;
        };
        $scope.canSubmit = function () {
            return $scope.material_login_form.$valid && !angular.equals($scope.tax, original);
        };
        ...
        $scope.viewTax = function (rowData) {
            $scope.show = 2;            
			$scope.tax  = angular.copy(rowData);
            original = angular.copy($scope.tax );
        }
        ...
        function getAllTax(selectedId) {
            var result = service.serverGet(config.urlGetTax);
            result.then(function (resolve) {
        ...
        $scope.saveTax = function () {
            var obj = this.tax;
            if (obj.id == null) {
                var result = service.serverPost(config.urlSaveTax, taxModel.add, commonModel.trans, obj)
                result.then(function (resolve) {
                    

                    if (resolve.ResponseData > 0) {
                        materials.displayToast(appConstants.successClass, appConstants.tax + ' ' + appConstants.saveMsg);
                        $scope.tax = {};
                        getAllTax(selectedId);
                    }
                    else {
                        materials.displayToast(appConstants.successClass, appConstants.tax + ' ' + appConstants.existMsg);
                    }
                ...
                obj.status = 1;
                var result = service.serverPost(config.urlUpdateTax, taxModel.edit, commonModel.trans, obj)
                result.then(function (resolve) {


                    if (resolve.ResponseData > 0) {
                        materials.displayToast(appConstants.successClass, appConstants.tax + ' ' + appConstants.updateMsg);
                        getAllTax(selectedId);
                        $scope.show = 1;
                    }
                    else {
        ...
        $scope.loadDropDown = function (id, varId) {
            ...
            var ddlistModel = taxModel.ddlist;
        ...
        function handleFile(e) {
            debugger;
            $scope.isvalid = false;
            var _keyset = appConstants.TAX_COL;
                                ...
                                if ($scope.uploadedItems.length > 0) {
                                    ...
                                    if (_keysetcount == columncount) {
                                        ...
                                        for (var m = 0; m < $scope.uploadedItems.length; m++) {
                                            ...
                                            var newtarget = {};
                                            newtarget.taxname = $scope.uploadedItems[m]["Tax"];
                                            newtarget.taxcode = $scope.uploadedItems[m]["Code"];
                                            newtarget.taxpercent = $scope.uploadedItems[m]["Percentage"];
        ...
        $scope.savebulkitem = function () {
            ...
            var result = service.serverPost(config.url_bulklookup, taxModel.bulk_add, "", obj)
            result.then(function (resolve) {
                debugger;
                materials.displayToast(appConstants.successClass, appConstants.tax + ' ' + appConstants.saveMsg);
