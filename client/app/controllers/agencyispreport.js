(function () {
    ...
    function AgencyISPReportCtrl($q, $scope, $http, $filter, $mdDialog, config, appConstants, service, materials, reportModel, commonModel) {
        ...
        var original;
        var init;
        $scope.listusertype = [{ id: 0, name: "All" }, { id: 2, name: "Agency" }, { id: 3, name: "ISP" } , { id: 6, name: "AR" }];
        ...
        $scope.search = search;
        $scope.order = order;
        $scope.numPerPageOpt = [50, 100, 300, 500,1000];
        ...
        $scope.loadDropDown = function (id, varId) {
            ...
            for (var key in ddlistModel) {
                ...
                if (varId != '' && valParam[1] != varId && valParam[2] != varId) {
                    continue;
                }
        ...
        $scope.runReport = function () {
            ...
            obj.stateid = $scope.agencyreport.stateid == undefined ? 0 : $scope.agencyreport.stateid;
        ...
        function selectedItemChange(flag) {
            ...
            $scope.agencyreport.stateid = $scope.selectedstateitem != undefined ? $scope.selectedstateitem.id : 0;
            ...
            if (flag == "S") {
                debugger;
                $scope.loadDropDown($scope.agencyreport.stateid, 'stateid');
            }
            if (flag == "C") {
                debugger;   
        ...
        $scope.cols = ["UserType","Agency Name", "First Name", "Last Name", "Constitution", "Gender", "Register Date", "Email Address", "Contact Type", "Contact No", "Landline", "Alternative contact person", "Alternative contact mobile No", "Nationality", "Country", "State", "City", "Area", "Pin Code", "Address", "User Name", "Password", "Skill", "Work Experience", "Preferred Locations", "Proofs", "Account holder name", "Bank Name", "Account Number", "Branch", "IFSC Code","Status"];
        $scope.colstitle = ["usertypecode","companyname", "firstname", "lastname", "constitutionname", "gendername", "joindate", "email", "contacttypename", "mobileno", "telephoneno", "contactperson", "contactpersonmobile", "nationalityname", "countryname", "statename", "cityname", "areaname", "zipcode", "address", "username", "password", "userskill", "userexp", "userprefferloc", "userproof", "accholdername", "bankname", "accnumber", "branch", "ifsc", "userstatus"];
                    ...
                    for (var m = 0; m < $scope.colstitle.length; m++) {
                        for (var key in selectObjec) {
                            if (key === $scope.colstitle[m]) {
                                var _value = selectObjec[key];
                                switch (key) {
                                    case "joindate":
                                        var _val = materials.stockconvertdatetime(new Date(_value), "D");
                                        selectdData.push(_val);
                                        break;
                                    case "usertypecode" :
                                        var _usertype = _value == 'AGNS' ? "Agency": _value;
                                        selectdData.push(_usertype);
                                        break;
                                    case "companyname" :
                                        var _usertype = selectObjec["usertypecode"] == 'AR' ? selectObjec["createdby"]: _value;
                                        selectdData.push(_usertype);
                                        break;
                                    default:
                                        
                                         _value = _value != null && _value != undefined ? _value : "";
                                        selectdData.push(_value);
                                        break;
                                }
                            }
                        }
