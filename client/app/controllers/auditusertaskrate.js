(function () {
    ...
    angular.module('app.table')
        .controller('AuditUserTaskRateCtrl', ['$q', '$rootScope', '$scope', '$http', '$filter', '$uibModal', '$mdDialog', 'config', 'appConstants', 'service', 'materials', 'auditUserTaskRateModel', 'commonModel', AuditUserTaskRateCtrl]) 
    ...
    function AuditUserTaskRateCtrl($q, $rootScope, $scope, $http, $filter, $uibModal, $mdDialog, config, appConstants, service, materials, auditUserTaskRateModel, commonModel) {
        var selectedId = 0;
        var original;
        var init;
        this.task = auditUserTaskRateModel.add;
        $scope.rate = 0;
        ...
        function loadtask(selectedId) {
            ...
            var result = service.serverPost(config.urlGetTaskTargetUserRateByTask, auditUserTaskRateModel.searchall, "", obj)
        ...
        $scope.loadtarget = function (rowData) {
            ...
            //var result = service.serverPost(config.urlGetTaskinvtarget, auditUserTaskRateModel.target, "", obj)
            //result.then(function (resolve) {
            //    materials.hideSpinner();
            //    debugger;
        ...
        $scope.bulkSaveAudit = function(status){           
            $scope.selected.forEach(function (row, index) {
                // Create a new object with the existing properties and the 'createdby' property
                var newRow = {
                    createdby: sessionStorage.userid,
                    status: status,                   
                };
                const mergedObject = { ...row, ...newRow };
                // Replace the original object with the new one
                $scope.selected[index] = mergedObject;
            });
            
            $scope.savepayment(0, $scope.selected,status)
        }

        $scope.singleSaveAudit  = function(row,status){
            var tempArray = [];
            row.createdby = sessionStorage.userid;
            row.status = status;
            tempArray.push(row);
            $scope.savepayment(row.id,tempArray,status)
        }

        $scope.savepayment = function (id,ratelist,status) {
            debugger;
            materials.showSpinner();
            var obj = {};
            obj.id = id;
           
            obj.listinputs = ratelist;
            // obj.id = row.id;
            // obj.taskid = row.taskid;
            // obj.targetid = row.targetid;           
            // obj.createdby = sessionStorage.userid;
                        
            var result = service.serverPost(config.urlUpdateTaskTargetUserRateByTask, auditUserTaskRateModel.audit_save, "", obj)
            result.then(function (resolve) {
                debugger;
                materials.hideSpinner();
        ...
        $scope.loadDropDown = function (id, varId) {
            ...
            var ddlistModel = auditUserTaskRateModel.ddlist;
        ...
        $scope.cols = ["Project", "Task", "Target", "Isp/Agency", "Amount"];
        $scope.colstitle = ["projectname","taskname", "targetname", "username", "rate"];

        $scope.exceller = function () {
            debugger;
            $scope.fileName = "UserTaskTragetRate";
            $scope.exportData = [];

            $scope.exportData.push($scope.cols);
            for (var n = 0; n < $scope.resultData.length; n++) {

                var idx = $scope.items.indexOf($scope.resultData[n].id);
                if (idx > -1) {
                    var selectdData = [];
