                                ...
                                if ($scope.uploadedItems.length > 0) {
                                    ...
                                    if (_keysetcount == columncount) {
                                        ...
                                        for (var m = 0; m < $scope.uploadedItems.length; m++) {
                                            ...
                                            var newtarget = {};
                                            var _paymentmode = $scope.uploadedItems[m]["Payment Mode"];
                                            _paymentmode = _paymentmode != null ? $scope.ToCaptilize(_paymentmode).trim() : "";                                                                                  
                                                var _payment = $filter('filter')($scope.resultData, function(value){
                                                    return (value.paymentmodename.toUpperCase().trim() == _paymentmode.toUpperCase().trim());
                                                })[0];
                                            if(_payment == undefined){
                                                newtarget.paymentmodename = _paymentmode;
                                                newtarget.paymentmodecode = $scope.uploadedItems[m]["Code"].toUpperCase().trim();
