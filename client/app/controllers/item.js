                                ...
                                if ($scope.uploadedItems.length > 0) {
                                    ...
                                    if (_keysetcount == columncount) {
                                        ...
                                        for (var m = 0; m < $scope.uploadedItems.length; m++) {
                                            ...
                                            var newtarget = {};
                                            var _itemname = $scope.uploadedItems[m]["Item"];
                                            _itemname = _itemname != null ? $scope.ToCaptilize(_itemname).trim() : "";                                                                                  
                                                var _item = $filter('filter')($scope.resultData, function(value){
                                                    return (value.itemname.toUpperCase().trim() == _itemname.toUpperCase().trim());
                                                })[0];
                                            if(_item == undefined){
                                                newtarget.itemname = _itemname;
                                                newtarget.itemcode = $scope.uploadedItems[m]["Code"].toUpperCase().trim();
