(function () {
    ...
    angular.module('app.table')
        ...
        .controller('targetexcelCtrl', ['$q', '$rootScope', '$scope', '$uibModalInstance', '$filter', 'items', 'config', 'appConstants', 'service', 'materials', 'commonModel', targetexcelCtrl]);
    ...
    function TargetCtrl($q,$rootScope, $scope, $http, $filter, $mdDialog, config, appConstants, service, materials, targetModel, commonModel, $element, $uibModal) {
            ...
            $mdDialog.show(confirm).then(function () {
                ...
                result.then(function (resolve) {
                    ...
                    if (resolve.ResponseData > 0) {
                        ...
                        var _cons = resolve.ResponseData == -1 ? appConstants.targetm : "";
                        var msg = appConstants.waringMsg.replace(/#NAME#/g, appConstants.target).replace(/#REFNAME#/g, appConstants.tasktargetmapping);
        ...
        async function select(page) {
            ...
            getAllTarget(stateid, cityid, areaid,clientid,start,end);
        };
        ...
        function search() {
             $scope.filteredresultData = $scope.resultData.length > 0 ?$scope.resultData[0].totalcount:$scope.resultData.length;
             return $scope.currentPageresultData = $filter('filter')($scope.resultData, $scope.searchKeywords);
       ...
       function getAllTargets(stateid, cityid, areaid,clientid,start,end) {            
            var obj = {};
            obj.stateid = stateid;
            obj.cityid = cityid;
            ...
            obj.end = end;
            obj.active = "ALL";
            obj.targetName = $scope.searchKeywords;
            materials.hideSpinner();
            //materials.showSpinner();
            var result = service.serverPost(config.urlGetTarget, targetModel.searchall, "", obj)
            result.then(function (resolve) {
                materials.hideSpinner();
                $scope.resultDataList = resolve.ResponseData;               
            }, function (reject) {
                materials.hideSpinner();               
            });
        }

        function getAllTarget(stateid, cityid, areaid,clientid,start,end) {            
            var obj = {};
            obj.stateid = stateid;
            obj.cityid = cityid;
            obj.clientid = clientid;
            ...
            result.then(function (resolve) {
                ...
                $scope.filteredresultData = $scope.resultData.length > 0 ?$scope.resultData[0].totalcount:$scope.resultData.length;
                $scope.currentPageresultData = $scope.resultData;
            }, function (reject) {
                materials.hideSpinner();
                alert('Not Resolved')
            });
        }
        getAllTargets(stateid, cityid, areaid,clientid,0,10);
        select($scope.currentPage);
        ...
        function handleFile(e) {            
            $scope.isvalid = false;
            materials.showSpinner();
            var _keyset = appConstants.TARGET_POOL_SAMPLE_EL_COL;
                               ...
                               $scope.uploadedFailedTargets = [];
                                if ($scope.uploadedItems.length > 0) {                                   
                                    ...
                                    if (_keysetcount == columncount) {
                                        ...
                                        var newtask = {};
                                        var newtarget = {};                                                                                                                 
                                       // $scope.uploadedTargets = angular.copy($scope.uploadedItems);                                     
                                                                               

                                        // var filterstate = $scope.liststate.map(value => value.statename.toLowerCase().trim());
                                        // var filtercity = $scope.listallcity.map(value => value.cityname.toLowerCase().trim());
                                        // var filterarea = $scope.listallarea.map(value => value.areaname.toLowerCase().trim());
                                        
                                        // var StateNames= $.unique($scope.uploadedTargets.map(function (d) {return filterstate.includes(d.State.toLowerCase().trim())?  d.State.toLowerCase().trim(): "";}));
                                        // var CityNames= $.unique($scope.uploadedTargets.map(function (d) {return StateNames.includes(d.State.toLowerCase().trim()) && filtercity.includes(d.City.toLowerCase().trim())? d.City.toLowerCase().trim():""}));
                                        // var AreaNames= $.unique($scope.uploadedTargets.map(function (d) {return StateNames.includes(d.State.toLowerCase().trim()) && CityNames.includes(d.City.toLowerCase().trim()) && filterarea.includes(d.Area.toLowerCase().trim()) ? d.Area.toLowerCase().trim():"";}));

                                        // var arr1_keys = targetlist.map(value => value.companyname.toLowerCase().trim()+"_"+
                                        // value.statename.toLowerCase().trim()+"_"+ value.cityname.toLowerCase().trim()+
                                        // "_"+value.areaname.toLowerCase().trim()+"_"+ value.targetname.toLowerCase().trim());
                                         
                                        // $scope.uploadedTargets = $scope.uploadedTargets.map(function (d){return (filterstate.includes(d.State.toLowerCase().trim())
                                        //     && filtercity.includes(d.City.toLowerCase().trim()) && filterarea.includes(d.Area.toLowerCase().trim())) ? d:"";})
                                            
                                        // $scope.uploadedItems = $scope.uploadedTargets.filter(
                                        //     value => !arr1_keys.includes(value["Client"].toLowerCase().trim()+"_"+
                                        //     value["State"].toLowerCase().trim()+"_"+ value["City"].toLowerCase().trim()+
                                        //     "_"+value["Area"].toLowerCase().trim()+"_"+ value["Target"].toLowerCase().trim()));

                                         for (var m = 0; m < $scope.uploadedItems.length; m++) {
                                            var newtarget = {};
                                            newtarget.taskname = $scope.uploadedItems[m]["Task"];
                                            var _targetname = $scope.uploadedItems[m]["Target"].toLowerCase().trim();
                                            ...
                                            var _clientname = $scope.uploadedItems[m]["Client"];
                                            ...
                                            _clientname = _clientname != null ? _clientname.toLowerCase().trim() : "";
                                            var city = undefined;
                                            var area = undefined;
                                            ...
                                            if(state != undefined) 
                                                city = $filter('filter')($scope.listallcity, function (value) {
                                                    return value.stateid == state.id && value.cityname.toLowerCase().trim() == _cityname;
                                                })[0];
                                            if(city != undefined)
                                                area = $filter('filter')($scope.listallarea, function (value) {
                                                    return value.stateid == state.id && value.cityid == city.id && value.areaname.toLowerCase().trim() == _areaname;
                                                })[0];

                                            var client = $filter('filter')($scope.listclient, function (value) {
                                                return value.companyname.toLowerCase().trim() == _clientname;
                                            })[0];
                                            
                                            // var target = null;
                                            // target = $filter('filter')(targetlist, function (value) {
                                            //     return (value.clientid == client.id && 
                                            //     value.stateid == state.id && value.cityid == city.id && value.areaid == area.id
                                            //     && value.targetname.toLowerCase().trim() == _targetname);
                                            // })[0];                                            
                                           
                                            newtarget.stateid = state != undefined ? state.id :null;
                                            newtarget.cityid = city != undefined ? city.id : null;
                                            newtarget.areaid = area != undefined ? area.id : null;
                                            newtarget.clientid = client != undefined ? client.id : null;

                                            // var target = $filter('filter')($scope.resultDataList, function (value) {
                                            //     return (value.companyname.toLowerCase().trim() == _clientname && 
                                            //     value.statename.toLowerCase().trim() == _statename && value.cityname.toLowerCase().trim() == _cityname && 
                                            //     value.areaname.toLowerCase().trim() == _areaname && value.targetname.toLowerCase().trim() == _targetname)
                                            // })[0];                                            
                                           
                                            
                                            //newtarget.targetid = target != null ? target.id : undefined;
                                            ...
                                            newtarget.client = $scope.uploadedItems[m]["Client"];
                                            ...
                                            // if (targetlist != undefined && newtarget.targetid == undefined && newtarget.stateid != null && newtarget.cityid != null && newtarget.areaid != null && newtarget.clientid != null) {
                                            if (newtarget.stateid != null && newtarget.cityid != null && newtarget.areaid != null && newtarget.clientid != null) {    
                                                $scope.uploadedTargets.push(newtarget);
                                            }
                                            else{
                                                $scope.uploadedFailedTargets.push(newtarget);
                                            }                                
                                        ...
                                        var targetlist = $filter('filter')($scope.resultDataList, function (value) {
                                            return (value.companyname.toLowerCase().trim() == $scope.uploadedTargets[0]['client'].toLowerCase().trim());
                                        });   

                                        var arr1_keys = targetlist.map(value => (value.companyname+"_"+
                                            value.statename+"_"+ value.cityname+
                                            "_"+value.areaname+"_"+ value.targetname).toLowerCase().trim());
                                         
                                        $scope.listalltask = $scope.uploadedTargets.filter(
                                            value => !arr1_keys.includes((value["client"]+"_"+
                                            value["state"]+"_"+ value["city"]+
                                            "_"+value["area"]+"_"+ value["targetname"]).toLowerCase().trim()));
                                    }
                                    else {
                                        materials.hideSpinner();
                                        $scope.uploadedItems = [];
        ...
        $scope.sampleexceller = function () {
            debugger;
            $scope.fileName = "Traget";
            $scope.exportData = [];           
            $scope.exportData.push(appConstants.TARGET_POOL_SAMPLE_EL_COL);           
    ...
    function targetexcelCtrl($q,$rootScope, $scope, $uibModalInstance, $filter, items, config, appConstants, service, materials, commonModel) {
        
        $scope.listuploadtarget = [];
        $scope.listtask = [];
        $scope.listtask = items.task;
        $scope.listuploadtarget = items.target;
        $scope.listbinddata = [];
        ...
        function select(page) {
            var end, start;
            start = (page - 1) * $scope.numPerPages;
            end = start + $scope.numPerPages;
            $scope.currentPage = page;
            $scope.startIndex = start;
            return $scope.listbinddata = $scope.listuploadtarget.slice(start, end);
        }; 

        $scope.currentPage = 1;
        $scope.numPerPages = 10;
        $scope.select = select;
        select($scope.currentPage);
        ...
        $scope.uploadNewTarget = function () {                    
            var splitlistuploadtarget = splitList($scope.listuploadtarget,30000);
            $uibModalInstance.dismiss("cancel");
            materials.showSpinner();
            for(var [index,listuploadtarget] of  splitlistuploadtarget.entries()){
                var tempArray = [];
                for (var i = 0; i < listuploadtarget.length; i++) {
                    // var tempArray = [];
                    var inputobj = angular.copy(listuploadtarget[i]);
                    ...
                    res.clientid = inputobj.clientid;
                ...
                result.then(function (resolve) {
                    if(upindex == splitlistuploadtarget.length-1){
                        materials.displayToast(appConstants.successClass, appConstants.target + ' ' + appConstants.saveMsg);
                        materials.hideSpinner();
