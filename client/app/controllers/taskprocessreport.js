(function () {
    ...
    angular.module('app.table').controller('TaskProcessReportCtrl', ['$q', '$scope', '$http', '$filter', '$mdDialog', 'config', 'appConstants', 'service', 'materials', 'reportModel', 'commonModel', 'targetModel', TaskProcessReportCtrl]);

    function TaskProcessReportCtrl($q, $scope, $http, $filter, $mdDialog, config, appConstants, service, materials, reportModel, commonModel, targetModel) {
        ...
        $scope.print = function (rowData) {
          ...
          '<tr >' +
            ...
            for (var k = 0; k < $scope.resultData.length; k++) {
                ...
                '<td style="border-bottom: 1px solid #ddd;">' + $scope.resultData[k].bookingrefno + '</td>' +
                '<td style="border-bottom: 1px solid #ddd;">' + $scope.resultData[k].bookingsourcename + '</td>' +
                '<td style="border-bottom: 1px solid #ddd;">' + $scope.resultData[k].customername + '-' + $scope.resultData[k].mobileno + '</td>' +
                '<td style="border-bottom: 1px solid #ddd;">' + materials.stockconvertdatetime(new Date($scope.resultData[k].pickupdate), "C") + "-" + $scope.resultdata[k].deliverylocation + '</td>' +
                '<td style="border-bottom: 1px solid #ddd;">' + materials.stockconvertdatetime(new Date($scope.resultData[k].dropdate), "C") + "-" + $scope.resultdata[k].droplocation + '</td>' +
                '<td style="border-bottom: 1px solid #ddd;">' + $scope.resultData[k].bookingstatusname + '</td>' +
                '<td style="border-bottom: 1px solid #ddd;">' + isnull($scope.resultData[k].distance) + '</td>' +
                '<td style="border-bottom: 1px solid #ddd;">' + isnull($scope.resultData[k].duration) + '</td>' +
                 '<td style="border-bottom: 1px solid #ddd;">' + isnull($scope.resultData[k].amount) + '</td>' +
                '</tr>';
            }
