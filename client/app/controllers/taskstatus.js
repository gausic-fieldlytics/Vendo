(function () {
    ...
    angular.module('app.table').controller('TaskstatusCtrl', ['$q', '$scope', '$http', '$filter', '$mdDialog', 'config', 'appConstants', 'service', 'materials', 'taskstatusModel', 'commonModel', TaskstatusCtrl]);

    function TaskstatusCtrl($q, $scope, $http, $filter, $mdDialog, config, appConstants, service, materials, taskstatusModel, commonModel) {
		var selectedId = 0;
        var original;
        var init;
        this.taskstatus = taskstatusModel.add;
        $scope.access_permission = {};
        $scope.access_permission = materials.getaccess(9);
        $scope.deleteTaskstatus = function (obj) {

            var confirm = materials.deleteConfirm(appConstants.taskstatus);
            $mdDialog.show(confirm).then(function () {
                var delObj = {
                    id: obj.id
                };
                var result = service.serverDelete(config.urlDeleteTaskstatus, taskstatusModel.delete, commonModel.trans, delObj);
                result.then(function (resolve) {
                    
                    if (resolve.ResponseData > 0) {
                        materials.displayToast(appConstants.successClass, appConstants.taskstatus + ' ' + appConstants.deleteMsg);
                        getAllTaskstatus(selectedId);
                        
                    }
                    else {
                        var _cons = resolve.ResponseData == -1 ? appConstants.task : "";
                        var msg = appConstants.waringMsg.replace(/#NAME#/g, appConstants.taskstatus).replace(/#REFNAME#/g, _cons);
        ...
        $scope.editTaskstatus = function (rowData) {

            $scope.show = 2;
            $scope.taskstatus = angular.copy(rowData);
            original = angular.copy($scope.taskstatus);

        }

        $scope.revert = function () {
            $scope.taskstatus = angular.copy(original);
			$scope.taskstatus = {};
        ...
        $scope.canRevert = function () {
            return !angular.equals($scope.taskstatus, original) || !$scope.material_login_form.$pristine;
        };
        $scope.canSubmit = function () {
            return $scope.material_login_form.$valid && !angular.equals($scope.taskstatus, original);
        };
        ...
        $scope.viewTaskstatus = function (rowData) {
            $scope.show = 2;            
			$scope.taskstatus  = angular.copy(rowData);
            original = angular.copy($scope.taskstatus );
        }
        ...
        function getAllTaskstatus(selectedId) {
            materials.showSpinner();
            var result = service.serverGet(config.urlGetTaskstatus);
            result.then(function (resolve) {
                debugger;
        ...
        $scope.saveTaskstatus = function () {
            var obj = this.taskstatus;
            if (obj.id == null) {
                var result = service.serverPost(config.urlSaveTaskstatus, taskstatusModel.add, commonModel.trans, obj)
                result.then(function (resolve) {
                    
                    if (resolve.ResponseData > 0) {
                        materials.displayToast(appConstants.successClass, appConstants.taskstatus + ' ' + appConstants.saveMsg);
                        $scope.taskstatus = {};
                        getAllTaskstatus(selectedId);
                        $scope.show = 1;
                    }
                    else {
                        materials.displayToast(appConstants.successClass, appConstants.taskstatus + ' ' + appConstants.existMsg);
                    }
                ...
                obj.status = 1;
                var result = service.serverPost(config.urlUpdateTaskstatus, taskstatusModel.edit, commonModel.trans, obj)
                result.then(function (resolve) {


                    if (resolve.ResponseData > 0) {
                        materials.displayToast(appConstants.successClass, appConstants.taskstatus + ' ' + appConstants.updateMsg);
        ...
        $scope.loadDropDown = function (id, varId) {
            ...
            var ddlistModel = taskstatusModel.ddlist;
        ...
        function handleFile(e) {
            debugger;
            $scope.isvalid = false;
            var _keyset = appConstants.TASK_STATUS_COL;
                        ...
                        if ($scope.uploadedItems.length > 0) {
                            ...
                            if (_keysetcount == columncount) {
                                ...
                                for (var m = 0; m < $scope.uploadedItems.length; m++) {
                                    ...
                                    var newtarget = {};
                                    newtarget.taskstatusname = $scope.uploadedItems[m]["Task Status"];
                                    newtarget.taskstatuscode = $scope.uploadedItems[m]["Code"];
        ...
        $scope.savebulkitem = function () {
            ...
            materials.showSpinner();
            var result = service.serverPost(config.url_bulklookup, taskstatusModel.bulk_add, "", obj)
            result.then(function (resolve) {
                debugger;
                materials.displayToast(appConstants.successClass, appConstants.taskstatus + ' ' + appConstants.saveMsg);
                materials.hideSpinner();
