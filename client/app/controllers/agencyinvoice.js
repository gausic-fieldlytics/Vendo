(function () {
    ...
    angular.module('app.table')
        .controller('AgencyInvoiceCtrl', ['$q', '$scope', '$http', '$filter', '$uibModal', '$mdDialog', 'config', 'appConstants', 'service', 'materials', 'agencyinvoiceModel', 'commonModel', 'userModel', AgencyInvoiceCtrl])

    function AgencyInvoiceCtrl($q, $scope, $http, $filter, $uibModal, $mdDialog, config, appConstants, service, materials, agencyinvoiceModel, commonModel, userModel) {
        var selectedId = 0;
        var original;
        var init;
        ...
        function checkProfileAuditStatus() {
            var obj = {};
            obj.id = sessionStorage.userid;
            obj.userid = sessionStorage.userid;
            var result = service.serverPost(config.urlGetUserById, userModel.searchbyid, "", obj)
            return result.then(function (resolve) {
                var oResult = resolve.ResponseData;
                if (["1","2","6"].includes(oResult.userstatusid)) {
                    return "Profile is under review. Pending approval.";
                } else if (oResult.userstatusid == "4") {
                    return "Profile was rejected. Please resubmit for approval.";
                }                               
                else{
                    return "";
                }
        ...
        $scope.generateinvoice = function () {
            checkProfileAuditStatus().then(function(isProfileAudit) {
                if(isProfileAudit != ""){
                    materials.displayToast(appConstants.errorClass, "" + ' ' + isProfileAudit + " Please contact admin.");
                    alert(isProfileAudit + " Please contact admin.");
                    return;
                }
                else {
                    debugger;
