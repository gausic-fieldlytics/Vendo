(function () {
    ...
    angular.module('app.table').controller('SkillCtrl', ['$q', '$scope', '$http', '$filter', '$mdDialog', 'config', 'appConstants', 'service', 'materials', 'skillModel', 'commonModel', SkillCtrl]);

    function SkillCtrl($q, $scope, $http, $filter, $mdDialog, config, appConstants, service, materials, skillModel, commonModel) {
		var selectedId = 0;
        var original;
        var init;
        this.skill = skillModel.add;
        $scope.access_permission = {};
        $scope.access_permission = materials.getaccess(10);
        $scope.deleteSkill = function (obj) {

            var confirm = materials.deleteConfirm(appConstants.skill);
            $mdDialog.show(confirm).then(function () {
                var delObj = {
                    id: obj.id
                };
                var result = service.serverDelete(config.urlDeleteSkill, skillModel.delete, "", delObj);
                result.then(function (resolve) {
                    
                    if (resolve.ResponseData > 0) {
                        materials.displayToast(appConstants.successClass, appConstants.skill + ' ' + appConstants.deleteMsg);
                        getAllSkill(selectedId);
                    }
                    else {
                        var _cons = resolve.ResponseData == -1 ? appConstants.agent : "";
                        var msg = appConstants.waringMsg.replace(/#NAME#/g, appConstants.skill).replace(/#REFNAME#/g, _cons);
        ...
        $scope.editSkill = function (rowData) {

            $scope.show = 2;
            $scope.skill = angular.copy(rowData);
            original = angular.copy($scope.skill);

        }

        $scope.revert = function () {
            $scope.skill = angular.copy(original);
			$scope.skill = {};
        ...
        $scope.canRevert = function () {
            return !angular.equals($scope.skill, original) || !$scope.material_login_form.$pristine;
        };
        $scope.canSubmit = function () {
            return $scope.material_login_form.$valid && !angular.equals($scope.skill, original);
        };
        ...
        $scope.viewSkill = function (rowData) {
            $scope.show = 2;            
			$scope.skill  = angular.copy(rowData);
            original = angular.copy($scope.skill );
        }
        ...
        function getAllSkill(selectedId) {
            materials.showSpinner();
            var result = service.serverGet(config.urlGetSkill);
            result.then(function (resolve) {
        ...
        $scope.saveSkill = function () {
            ...
            obj.status = 1;
            if (obj.id == null) {
                var result = service.serverPost(config.urlSaveSkill, skillModel.add, "", obj)
                result.then(function (resolve) {
                    ...
                    if (resolve.ResponseData > 0) {
                        ...
                        materials.displayToast(appConstants.successClass, appConstants.skill + ' ' + appConstants.existMsg);
                    }
                ...
                var result = service.serverPost(config.urlUpdateSkill, skillModel.edit, "", obj)
                result.then(function (resolve) {

                    if (resolve.ResponseData > 0) {
                        materials.displayToast(appConstants.successClass, appConstants.skill + ' ' + appConstants.updateMsg);
                        getAllSkill(selectedId);
                        $scope.show = 1;
                    }
                    else {
        ...
        $scope.loadDropDown = function (id, varId) {
            ...
            var ddlistModel = skillModel.ddlist;
        ...
        function handleFile(e) {
            debugger;
            $scope.isvalid = false;
            var _keyset = appConstants.SKILL_COL;
                                ...
                                if ($scope.uploadedItems.length > 0) {
                                    ...
                                    if (_keysetcount == columncount) {
                                        ...
                                        for (var m = 0; m < $scope.uploadedItems.length; m++) {
                                            ...
                                            var newtarget = {};
                                            var _skillname = $scope.uploadedItems[m]["Skill"];
                                            _skillname = _skillname != null ? $scope.ToCaptilize(_skillname).trim() : "";                                                                                  
                                                var _payment = $filter('filter')($scope.resultData, function(value){
                                                    return (value.skillname.toUpperCase().trim() == _skillname.toUpperCase().trim());
                                                })[0];
                                            if(_payment == undefined){
                                                newtarget.skillname = $scope.uploadedItems[m]["Skill"];
                                                newtarget.skillcode = $scope.uploadedItems[m]["Code"];
        ...
        $scope.savebulkitem = function () {
            ...
            materials.showSpinner();
            var result = service.serverPost(config.url_bulklookup, skillModel.bulk_add, "", obj)
            result.then(function (resolve) {
                debugger;
                materials.displayToast(appConstants.successClass, appConstants.skill + ' ' + appConstants.saveMsg);
                materials.hideSpinner();
