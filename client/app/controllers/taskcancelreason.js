(function () {
    ...
    function TaskcancelreasonCtrl($q, $scope, $http, $filter, $mdDialog, config, appConstants, service, materials, taskcancelreasonModel, commonModel) {
        ...
        $scope.saveTaskcancelreason = function () {
            ...
            var _itemname = obj.taskcancelreasonname.trim();                                                                                         
            var _item = $filter('filter')($scope.resultData, function(value){
                return (value.taskcancelreasonname.toUpperCase().trim() == _itemname.toUpperCase().trim());
            })[0];
            if(_item == undefined){
                if (obj.id == null) {
                    ...
                    result.then(function (resolve) {
                        ...
                        if (resolve.ResponseData > 0) {
                            ...
                            materials.displayToast(appConstants.successClass, appConstants.taskcancelreason + ' ' + appConstants.existMsg);
                        }                        
                                ...
                                if ($scope.uploadedItems.length > 0) {
                                    ...
                                    if (_keysetcount == columncount) {
                                        ...
                                        for (var m = 0; m < $scope.uploadedItems.length; m++) {
                                            ...
                                            if(_item == undefined){
                                                newtarget.taskcancelreasonname = _itemname;
