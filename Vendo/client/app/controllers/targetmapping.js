(function () {
    ...
    angular.module('app.table')
        .controller('TargetMappingCtrl', ['$q', '$rootScope', '$scope', '$http', '$filter', '$mdDialog', 'config', 'appConstants', 'service', 'materials', 'targetModel', 'commonModel', '$element', '$uibModal', TargetMappingCtrl]);

    function TargetMappingCtrl($q,$rootScope, $scope, $http, $filter, $mdDialog, config, appConstants, service, materials, targetModel, commonModel, $element, $uibModal) {
        var selectedId = 0;
        var original;
        var init;
        ...
        function loadtarget(flag) {
            ...
            result.then(function (resolve) {
                ...
                for (var i = 0; i < $scope.listtarget.length; i++) {
                    $scope.items_target.push($scope.listtarget[i].id);
        ...
        function getAllTargetapping(taskid) {
            ...
            result.then(function (resolve) {
                ...
                for (var i = 0; i < oResult.length; i++) {
                    ...
                    $scope.selected_target.push(oResult[i].targetid);
                }
